 ...
 */
Asset.controller('assetUsedCtrl', ['$ionicLoading','$scope', '$state', 'assetUsedService','$ionicHistory','$rootScope','DataService','UrlService','PopupService','$localstorage','$stateParams','$ionicPopup',
  function ($ionicLoading,$scope, $state, assetUsedService,$ionicHistory,$rootScope,DataService,UrlService,PopupService,$localstorage,$stateParams,$ionicPopup) {
    //进入页面后初始化页面
    ...
    var getData =function() {
      ...
      // 调用接口
      assetUsedService.progressBar(param)
      ...
      // 调用接口
      assetUsedService.notProgressBar(notParam)
        .success(function (response) {
          console.log('未完成工作流');
          console.log(response);
          if (response.code == 200) {
    ...
    $scope.goApproval = function () {
      ...
      DataService.setData("assetUsedApprove",$scope.assetUsedApply);
    ...
    $scope.approvalBill = function () {
      ...
      // 调用接口
      assetUsedService.approvalBill(param)
        .success(function (response) {
          console.log('审批领用单');
          console.log(response);
          if (response.code == 200) {
...
Asset.service('assetUsedService', ['$http', 'UrlService',function ($http, UrlService) {
