 ...
 */
Asset.controller('selectAssetTypeCtrl', ['$scope', '$state', '$ionicHistory', '$ionicLoading', 'selectAssetTypeService', 'DataService', 'UrlService', 'PopupService', '$localstorage', '$stateParams', 'UserService',
  function ($scope, $state, $ionicHistory, $ionicLoading, selectAssetTypeService, DataService, UrlService, PopupService, $localstorage, $stateParams, UserService) {
    //进入页面后初始化页面
   ...
   */
    $scope.selected = function (e) {
      ...
      DataService.clearData("assetname");//清除缓存
      DataService.clearData("assetcode");//清除缓存
      DataService.setData("assetname", e.sortName);
      DataService.setData("assetcode", e.id);
      $ionicHistory.goBack();
    },
      //获取数据字典
      $scope.getDate = function () {
        ...
        selectAssetTypeService.getData1(params)
...
Asset.service('selectAssetTypeService', ['$http', 'UrlService', function ($http, UrlService) {
  //获取资产类别列表
  this.getData1 = function (params) {
    return $http.post(UrlService.getUrlData_check('ASSET_ASSETSORTLIST'), params);
  }
}]);
