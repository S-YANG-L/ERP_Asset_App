 ...
 */
Asset.controller('selectAssetTopCtrl', ['$scope', '$state', '$ionicHistory', '$ionicLoading', 'selectAssetTopService', 'DataService', 'UrlService', 'PopupService', '$localstorage', '$stateParams', 'UserService',
  function ($scope, $state, $ionicHistory, $ionicLoading, selectAssetTopService, DataService, UrlService, PopupService, $localstorage, $stateParams, UserService) {
    //进入页面后初始化页面
   ...
   */
    $scope.selected = function (e) {
      console.log(e)
      DataService.clearData("assetTopname");//清除缓存
      DataService.clearData("assetTopcode");//清除缓存
      DataService.setData("assetTopname", e.sortName);
      DataService.setData("assetTopcode", e.sortCode);
      $ionicHistory.goBack();
    },
      //获取数据字典
      $scope.getDate = function () {
        ...
        selectAssetTopService.getData1(params)
...
Asset.service('selectAssetTopService', ['$http', 'UrlService', function ($http, UrlService) {
  //获取资产类别列表
  this.getData1 = function (params) {
    return $http.post(UrlService.getUrlData_check('ASSET_ASSETTOPSORTLIST'), params);
  }
}]);
