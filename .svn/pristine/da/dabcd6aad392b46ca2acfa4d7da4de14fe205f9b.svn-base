 ...
 */
Asset.controller('useRefundDoneCtrl', ['$scope', '$state', 'useRefundDoneService','$ionicHistory','$rootScope','DataService','UrlService','PopupService','$localstorage','$stateParams','$ionicPopup',
  function ($scope, $state, useRefundDoneService,$ionicHistory,$rootScope,DataService,UrlService,PopupService,$localstorage,$stateParams,$ionicPopup) {
    //进入页面后初始化页面
    ...
    $scope.delete=function(){
      console.log('数据跳转测试');
      console.log($scope.useRefundDone.approvalStatus);
      if($scope.useRefundDone.approvalStatus=='0'){
        $scope.goDeleate();
      }else{
        ...
        useRefundDoneService.revokeBillApply(param)
    ...
    $scope.goDeleate = function () {
      ...
      useRefundDoneService.deleteForm(params)
        .success(function (response) {
          // 回调成功,处理返回值
          console.log('单据删除');
          console.log(response);
    ...
    $scope.Submit=function(){
      ...
      if($scope.useRefundDone.approvalStatus=='0'){
        ...
        DataService.setData("outBoundCache",$scope.useRefundDone);
        $state.go('useRefundDone');
      }else{
        var billInfo = DataService.getData("CurrBillInfo");//当前操作的单据
        var param = {
          sid: $localstorage.getObject('sid'),
    ...
    var getData =function() {
      ...
      // 调用接口
      useRefundDoneService.progressBar(param)
        .success(function (response) {
          console.log('资产退库单',response);
          if (response.code == 200) {
            $scope.useRefundDone = response.list;
      ...
      // 调用接口
      useRefundDoneService.notProgressBar(notParam)
        .success(function (response) {
    ...
    $scope.againApproveBorrow=function(){
      ...
      useRefundDoneService.revokeApprove(param)
        .success(function (response) {
          console.log('领用单重新审批');
          console.log(response);
          if (response.code == 200) {
            $state.go('remind.todo_list');
          } else if (response.code == 201) {
            PopupService.showToast(response.message);
          } else {
            PopupService.showToast(response.message);
...
Asset.service('useRefundDoneService', ['$http', 'UrlService',function ($http, UrlService) {
